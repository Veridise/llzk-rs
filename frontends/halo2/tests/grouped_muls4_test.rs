...
(assert (= adv_2_2 adv_0_3))
(assert (= adv_2_3 adv_0_4))
...
#[test]
fn grouped_muls4_circuit_picus() {
    common::setup();
    common::picus_test(
...
#[test]
fn grouped_muls4_opt_circuit_picus() {
    common::setup();
    common::picus_test(
        MulCircuit::<Fr>::default(),
...
impl<F: Field> MulChip<F> {
    ...
    pub fn configure(meta: &mut ConstraintSystem<F>) -> MulConfig {
        ...
        meta.create_gate("mul3", |meta| {
            ...
            Constraints::with_selector(selector3, vec![a * b - c])
        });
