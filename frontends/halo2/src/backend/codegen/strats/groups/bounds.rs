...
impl<'a> GroupBounds<'a> {
    pub fn new<F: Field>(group: &'a Group, ctx: super::GroupIRCtx<'_, '_, F>) -> Self {
        Self::new_with_extra(group, ctx, None)
    }

    pub fn new_with_extra<F: Field>(
        group: &'a Group,
                ...
                match i {
                    GroupCell::Assigned(cell) => {
                        if !region_indices.contains(&cell.region_index) {
                            ...
                            // rows.
                            let abs_row = cell.row_offset
                                + ctx.regions_by_index[&cell.region_index].start()?;
