use crate::{
    backend::codegen::strats::groups::bounds::{Bound, EqConstraintCheck, GroupBounds},
    halo2::{Any, Column, Field},
    synthesis::groups::{Group, GroupCell},
};
use anyhow::Result;
...
pub fn lift_free_cells_to_inputs<F: Field>(
    ...
    while !worklist.is_empty() {
        ...
        for (caller_idx, caller) in groups.iter().enumerate() {
            ...
            let bounds = GroupBounds::new(caller, ctx);
...
fn find_free_cells<F: Field>(
    ...
    let bounds = GroupBounds::new(group, ctx);
