...
impl LookupCodegenStrategy for InvokeLookupAsModule {
        ...
        C: Codegen<'c>,
    {
        for lookup in Lookup::load(syn)? {
            let scope = lookup.create_scope(codegen, syn)?;
            ...
            codegen.lower_stmts(&scope, assumptions.chain(constraints))?;
            codegen.on_scope_end(scope)?;
        }
        Ok(())
    }
    ...
    fn invoke_lookups<L>(
        ...
        scope: &L,
        syn: &CircuitSynthesis<L::F>,
    ) -> Result<impl Iterator<Item = anyhow::Result<CircuitStmt<L::CellOutput>>>>
    where
        L: Lowering,
    {
        ...
        Ok(Lookup::load(syn)?
            .into_iter()
            .flat_map(move |l| region_rows().map(move |r| l.create_call_stmt(scope, &r))))
    }
}
